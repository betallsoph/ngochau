...
var POST = async ({ request }) => {
	try {
		const { landlordId, propertyId, month, dueDate, readings } = await request.json();
		...
		if (rooms.length === 0) return json({ error: "Không tìm thấy phòng có khách đang ở trong tòa nhà này" }, { status: 400 });
		const createdInvoices = await db.$transaction(async (tx) => {
			...
			for (const room of rooms) {
				...
				for (const item of invoiceItems) await tx.invoiceItem.create({ data: {
					invoiceId: inv.id,
					name: item.name,
					amount: item.amount,
