...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let properties = [];
		$$renderer.push(`<div class="space-y-6"><div class="flex items-center justify-between"><div><h1 class="text-2xl font-black text-black">Danh Sách Tòa Nhà</h1> <p class="text-zinc-600 text-sm mt-1 font-bold">${escape_html(properties.length)} tòa nhà, ${escape_html(properties.reduce((sum, p) => sum + p.rooms.length, 0))} phòng trọ</p></div> <button class="bg-blue-300 text-black border-2 border-black px-4 py-2.5 rounded-[6px] shadow-secondary hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all flex items-center gap-1.5 cursor-pointer font-black text-sm">Thêm tòa nhà `);
